...
(defn post-process-fn
  ...
  (cond (#{"alpha"} module) (render-module data)))
...
(defn build-fn [{:keys [::module ::profile] :as opts}]
  (binding [*out* (java.io.StringWriter.)]
    (new/create {:template "amiorin/big-infra-v2"
                 :name "amiorin/big-infra-v2"
...
(defn run-steps
  ...
  ([step-fns opts steps cmds module profile]
   (let [opts (-> "amiorin/big_infra_v2/config.edn"
                  io/resource
                  aero/read-config
