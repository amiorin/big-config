import { createStepFn, ok } from "./core.js";
...
import { ENV, ERR, EXIT, RUN_CMDS, RUN_CMD, RUN_SHELL_OPTS, RENDER_MODULE, RENDER_PROFILE, RENDER_TEMPLATES, WF_CREATE_FN, WF_CREATE_OPTS, WF_BUILD_FN, WF_BUILD_OPTS, WF_DELETE_FN, WF_DELETE_OPTS, WF_DESCRIBE_FN, WF_NAME, WF_OBJECT_FN, WF_OBJECT_PREFIX, WF_PARAMS, WF_PATH_FN, WF_PREFIX, WF_STEPS, WF_VALIDATE_FN, WORKFLOW_NS, addSuffix, namespaceOf, nameOf, qualify, selectKeys } from "./keys.js";
...
export const printStepFn = createStepFn({
    beforeF: (step, opts) => {
        const failed = opts[EXIT] != null && opts[EXIT] !== 0;
        const prefix = failed ? "✖" : "➜";
        let msg;
        if (step === LOCK)
            msg = `Lock (owner ${opts["big-config.lock/owner"] ?? ""})`;
        else if (step === UNLOCK_ANY)
            msg = "Unlock any";
        else if (step === GIT_CHECK)
            msg = "Checking if the working directory is clean";
        else if (step === RENDER)
            msg = `Rendering workflow: ${opts[WF_NAME] ?? ""}`;
        else if (step === RUN_CMD)
            msg = `Running:\n> ${(opts[RUN_CMDS] ?? [])[0] ?? ""}`;
        if (msg)
...
function resolveFn(key, opts, defaultValue) {
    const f = opts[key];
    if (f == null) {
...
export function selectGlobals(opts) {
...
export function runSteps(stepFns, opts) {
    ...
    let optsStar = opts;
    let queuedSteps = (opts[WF_STEPS] ?? []).map((step) => namespaceOf(step) ? step : qualify(WORKFLOW_NS, step));
    const wf = pluggableWorkflowStar({
        ...
        wireFn: (step) => {
            switch (step) {
                ...
                case LOCK: return [(o) => lock(stepFns, o), undefined];
                case GIT_CHECK: return [(o) => gitCheck(stepFns, o), undefined];
                case RENDER: return [(o) => renderTemplates(stepFns, o), undefined];
                case CREATE: return [(o) => resolveFn(WF_CREATE_FN, opts)(stepFns, o), undefined];
                case BUILD: return [(o) => resolveFn(WF_BUILD_FN, opts)(stepFns, o), undefined];
                case DELETE: return [(o) => resolveFn(WF_DELETE_FN, opts)(stepFns, o), undefined];
                case VALIDATE: return [(o) => resolveFn(WF_VALIDATE_FN, opts, (_s, x) => ok(x))(stepFns, o), undefined];
                case DESCRIBE: return [(o) => resolveFn(WF_DESCRIBE_FN, opts, (_s, x) => ok(x))(stepFns, o), undefined];
                case EXEC: return [(o) => runCmds(stepFns, o), undefined];
                case GIT_PUSH: return [gitPush, undefined];
                case UNLOCK_ANY: return [(o) => unlockAny(stepFns, o), undefined];
                case END: return [(x) => x, undefined];
                default: return [(x) => x, undefined];
            }
        },
        nextFn: (step, _nextStep, stepOpts) => {
            ...
            if (step === END)
                return [undefined, optsStar];
            if (stepOpts[EXIT] > 0)
                return [END, optsStar];
            const nextStep = queuedSteps[0];
            queuedSteps = queuedSteps.slice(1);
            if (nextStep)
                return [nextStep, nextStep === CREATE ? createOpts : nextStep === BUILD ? buildOpts : nextStep === DELETE ? deleteOpts : optsStar];
            return [END, optsStar];
        }
    });
...
function tokenize(strOrArgs) {
    if (Array.isArray(strOrArgs))
        return [...strOrArgs];
    const trimmed = strOrArgs.trim();
    return trimmed.length === 0 ? [] : trimmed.split(/\s+/);
}
export function parseArgs(strOrArgs) {
    ...
    while (i < xs.length) {
        ...
        if (parseArgSteps.has(tokenName) && !namespaceOf(token)) {
            ...
            if (rest.length === 0)
                throw Object.assign(new Error("-- cannot be without a command"), { data: {} });
            if (!steps.includes("exec"))
                steps.push("exec");
            cmds.push(rest.join(" "));
            i = xs.length;
        }
...
function buildPath(parts, profile, suffix) {
    return [...parts, `${profile}-${suffix}`].join("/");
}
export function newPrefix(opts, firstStep) {
...
export function path(opts, name) {
    return `${opts[WF_PREFIX] ?? ".dist"}/${keywordToPath(name)}`;
}
export function prepare(opts, overrides) {
    ...
    const pathFn = overrides[WF_PATH_FN] ?? ((o) => `${prefix ?? ".dist"}/${keywordToPath(o[WF_NAME])}`);
    const objectFn = overrides[WF_OBJECT_FN] ?? ((o) => `${objectPrefix ?? "tofu"}/${keywordToName(o[WF_NAME])}`);
    const merged = { ...opts, ...overrides };
    const dir = pathFn(merged);
    const object = objectFn(merged);
    const params = overrides[WF_PARAMS] ?? {};
    const templates = (merged[RENDER_TEMPLATES] ?? []).map((tpl) => ({ ...tpl, ...params, "target-dir": dir, "target-object": object }));
    return {
        ...merged,
...
export function mergeParams(tools, params, opts) {
    let out = clone(opts);
...
export function readBcPars(opts, env = process.env) {
    const paramsFromEnv = {};
    for (const [k, v] of Object.entries(env)) {
        if (!k.startsWith(ENV_PREFIX) || v == null)
            continue;
...
export function unregisterWorkflow(name) {
...
export function createWorkflowStar(options) {
    ...
    if (!Array.isArray(pipeline))
        throw new TypeError(":pipeline must be like [::tool/tofu ...\n::tool/ansible ...");
    return ((stepFns, opts) => {
        ...
        const stepToOptsAndFn = new Map();
        for (let i = 0; i < pipeline.length; i += 2) {
            const step = pipeline[i];
            const tuple = pipeline[i + 1] ?? [];
            const args = tuple[0] ?? [];
            const optsFn = tuple[1] == null ? ((x) => x) : toFn(tuple[1]);
            const stepOpts = opts[addSuffix(step, "-opts")] ?? {};
            stepToOptsAndFn.set(step, [{ ...parseArgs(args), ...globalsOpts, ...stepOpts }, optsFn]);
        }
        const steps = pipeline.filter((_, i) => i % 2 === 0);
        const stepsSet = new Set(steps);
        const stepToNext = new Map();
        const sequence = [firstStep, ...steps, actualLastStep, undefined];
        for (let i = 0; i < sequence.length - 1; i++)
            stepToNext.set(sequence[i], sequence[i + 1]);
        let optsStar = opts;
        const stepToFn = (step) => {
            if (step === firstStep)
                return ok;
            if (step === actualLastStep)
                return (x) => x;
            const registered = workflowRegistry.get(step) ?? opts[step];
            if (typeof registered !== "function")
                throw Object.assign(new Error(`Workflow '${step}' is not registered`), { data: { step } });
            return (o) => registered(stepFns, o);
        };
        const wf = pluggableWorkflowStar({
            firstStep,
            ...
            nextFn: (step, nextStep, stepOpts) => {
                ...
                if (step === actualLastStep || step === END)
                    return [undefined, optsStar];
                if (stepOpts[EXIT] > 0)
                    return [actualLastStep, optsStar];
                const [newOpts, optsFn] = stepToOptsAndFn.get(nextStep) ?? [optsStar, (x) => x];
...
registerFunction("big-config.workflow/run-steps", runSteps);
