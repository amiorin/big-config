...
import { ENV, RUN_CMDS, WF_STEPS, STEP_MODULE, STEP_PROFILE } from "./keys.js";
...
export function parse(s) {
    ...
    while (i < xs.length) {
        ...
        if (token === "--") {
            ...
            if (cmds.length > 0)
                cmds = cmds.map((cmd) => `${cmd}${globalArgs ? `:${globalArgs}` : ""}`);
            else if (globalArgs)
                cmds = [globalArgs];
...
export function parseModuleAndProfile(s) {
    const [, , module, profile] = parse(s);
    return { module, profile };
}
export function runStep(stepFns, opts) {
    let current = opts;
    for (const step of opts[WF_STEPS] ?? []) {
        switch (step) {
            case "lock":
                current = lock(stepFns, current);
                break;
            case "git-check":
                current = gitCheck(stepFns, current);
                break;
            case "build":
                current = ok(current);
                break; // Legacy build step placeholder; no build helper is part of this rewrite.
            case "render":
                current = renderTemplates(stepFns, current);
                break;
            case "exec":
                current = runCmds(stepFns, current);
                break;
            case "git-push":
                current = gitPush(current);
                break;
            case "unlock-any":
                current = unlockAny(stepFns, current);
                break;
            default: current = ok(current);
        }
        if ((current["big-config/exit"] ?? 0) > 0)
            break;
    }
    return current;
}
export function runSteps(s, opts = { [ENV]: "repl" }, stepFns) {
