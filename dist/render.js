...
import { render as selmerRender, withoutEscaping } from "selmer";
import "./selmer-filters.js";
import { createWorkflow } from "./core.js";
import { RENDER_MODULE, RENDER_PROFILE, RENDER_TEMPLATES, STEP_MODULE, STEP_PROFILE } from "./keys.js";
import { normalizeDelimiters, whitespaceControl } from "./selmer-filters.js";
...
function selmer(input, data, delimiters) {
...
function walkFiles(dir) {
    const out = [];
    if (!existsSync(dir))
        throw Object.assign(new Error(`Template directory not found: ${dir}`), { data: { dir } });
...
export function copyDir({ srcDir, targetDir, data, delimiters }) {
    for (const srcFile of walkFiles(srcDir)) {
        ...
        try {
            chmodSync(targetFile, statSync(srcFile).mode);
        }
        catch { /* best effort */ }
    }
}
...
function parseTransform(spec) {
    ...
    if (rest[0] instanceof Map)
        ...
        if (delimiterKeys.some((k) => k in maybe))
            delimiters = rest.shift();
        else
            files = rest.shift();
    }
    if (!delimiters && isDelimiterObject(rest[0]))
        delimiters = rest.shift();
...
export function copyTemplateDir(args) {
    ...
    for (const [from, to] of files) {
        ...
        if (!only)
            rmSync(join(targetBase, from), { force: true });
        ensureParent(targetFile);
        const replaceable = !raw && !nonReplacedExts.has(fileExt(srcFile));
        if (replaceable)
            writeFileSync(targetFile, selmer(readFileSync(srcFile, "utf8"), data, args.delimiters));
        else
            cpSync(srcFile, targetFile);
        try {
            chmodSync(targetFile, statSync(srcFile).mode);
...
export function resolveTemplateDir(template) {
...
function templateTargetDir(edn) {
    return edn["target-dir"] ?? edn.targetDir;
}
function getMultiOption(value) {
    if (value == null)
        return [];
    return Array.isArray(value) ? value : [value];
}
export function render(opts) {
    const templates = opts[RENDER_TEMPLATES];
    if (templates == null)
        throw new TypeError(":big-config.render/templates should never be nil");
    for (const inputEdn of templates) {
        const dataFn = toFn(inputEdn["data-fn"] ?? inputEdn.dataFn, (data) => data);
        const templateFn = toFn(inputEdn["template-fn"] ?? inputEdn.templateFn, (_data, edn) => edn);
        const dataBase = {};
        for (const [k, v] of Object.entries(inputEdn))
        ...
        if (!template || !targetDir)
            throw Object.assign(new Error("Invalid template"), { data: { edn } });
        if (!edn.transform)
            throw new TypeError(":transform not defined");
        if (edn.transform.length === 0)
            throw new TypeError(":transform is an empty list");
        const templateDir = resolveTemplateDir(template);
        if (existsSync(targetDir)) {
            if (edn.overwrite === "delete" || edn.overwrite === ":delete")
                rmSync(targetDir, { recursive: true, force: true });
            else if (!edn.overwrite)
                throw Object.assign(new Error(`${targetDir} already exists (and :overwrite was not true).`), { data: {} });
        }
        for (const spec of edn.transform) {
            const parsed = parseTransform(spec);
            copyTemplateDir({
                templateDir,
                targetDir,
                data,
...
export function discover(parentDir) {
    const out = [];
