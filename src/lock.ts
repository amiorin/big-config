...
import { ERR, EXIT, LOCK_DETAILS, LOCK_KEYS, LOCK_NAME, LOCK_OWNER, TAG_CONTENT, type Opts } from "./keys.js";
import { genericCmd } from "./run.js";
import { hashString, registerFunction, sortNestedMap, stableStringify } from "./utils.js";

function ednString(value: unknown): string {
  if (Array.isArray(value)) return `[${value.map(ednString).join(" ")}]`;
  if (value && typeof value === "object") {
    return `{${Object.entries(value as Opts).map(([k, v]) => `:${k} ${ednString(v)}`).join(" ")}}`;
  }
...
export function parseTagContent(tagContent: string): Opts {
  const line = tagContent.split(/\r?\n/).find((x) => x.startsWith(">>>"));
  if (!line) return {};
  const body = line.replace(/^>>>/, "").trim();
  if (!body) return {};
  try { return JSON.parse(body) as Opts; } catch { /* EDN fallback */ }
  const out: Opts = {};
  const inner = body.replace(/^\{/, "").replace(/\}$/, "");
  const re = /:([^\s\}]+)\s+("(?:\\.|[^"])*"|[^\s\}]+)/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(inner))) out[m[1]!] = parseScalar(m[2]!);
  return out;
}

export function generateLockId(opts: Opts): Opts {
  const lockKeys: string[] = opts[LOCK_KEYS] ?? [];
  const lockDetails: Opts = {};
  for (const k of lockKeys) if (Object.prototype.hasOwnProperty.call(opts, k)) lockDetails[k] = opts[k];
...
export function deleteTag(opts: Opts): Opts {
  return genericCmd({ opts, cmd: ["git", "tag", "-d", opts[LOCK_NAME]] });
}

export function createTag(opts: Opts): Opts {
  return genericCmd({
    opts,
...
export function checkRemoteTag(opts: Opts): Opts {
...
registerFunction("big-config.lock/lock", lock as any);
