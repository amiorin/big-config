...
export function normalizeKeyword(k: Keyword): Keyword {
  if (typeof k !== "string") return String(k);
  return k.startsWith(":") ? k.slice(1) : k;
}

export function keyword(nsOrName: string, name?: string): Keyword {
  const ns = normalizeKeyword(nsOrName);
  if (name === undefined) return ns;
  return `${ns}/${normalizeKeyword(name)}`;
}

export function namespaceOf(k: Keyword | undefined | null): string | undefined {
...
export function nameOf(k: Keyword | undefined | null): string {
  if (k == null) return "";
  const s = normalizeKeyword(k);
  const i = s.lastIndexOf("/");
...
export function qualify(defaultNs: string, k: Keyword): Keyword {
  const n = normalizeKeyword(k);
  return namespaceOf(n) ? n : keyword(defaultNs, nameOf(n));
}

export function addSuffix(k: Keyword, suffix: string): Keyword {
  const ns = namespaceOf(k);
  return ns ? keyword(ns, `${nameOf(k)}${suffix}`) : `${nameOf(k)}${suffix}`;
}

export function selectKeys<T extends Opts>(obj: T | undefined | null, keys: Iterable<string>): Opts {
  const out: Opts = {};
  if (!obj) return out;
  for (const k of keys) {
...
export function isPlainObject(value: unknown): value is Opts {
