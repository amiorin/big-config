        ...
        (if (= selected @cnt)
          [:tr
           [:td [:input {:style "min-width: max-content;"
                         :type "text", :data-bind:name true :data-init (format "$name = '%s'" name) :value name}]]
           [:td [:input {:style "min-width: max-content;"
                         :type "text", :data-bind:email true :data-init (format "$email = '%s'" email) :value email}]]
           [:td
            [:fieldset.grid {:style "display: flex;"}
             [:button.secondary {:data-on:click (str "$selected = " @cnt "; "
                                                     "$operation = 'cancel'; "
                                                     ""
